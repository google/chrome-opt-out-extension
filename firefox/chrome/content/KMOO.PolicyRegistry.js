...
KMOO.PolicyRegistry.loadDomainPolicyRegistry_ = function() {
  ...
  req.open('GET', 'chrome://kmoo/content/registry.json', true);
  req.onreadystatechange = function(e) {
    if (req.readyState === 4 && (req.status === 200 || req.status === 0)) {
      var nativeJSON = Components.classes['@mozilla.org/dom/json;1'].
          createInstance(Components.interfaces.nsIJSON);
      var temp = nativeJSON.decode(req.responseText);
      if (!temp.registry) {
      ...
      for (var i = temp.registry.length - 1; i >= 0; i--) {
        ...
        // Normalize to Firefox's nsICookie API values
        // Domains start with `.`.
        var domain = el.host;
        if (el.host[0] === '.') {
          el.isDomain = true;
          el.host = el.host.substr(1);
        } else {
          el.isDomain = false;
          domain = '.' + el.host;
        }
        ...
        if (!KMOO.PolicyRegistry.registry_.byDomain[domain]) {
          // Add the cookie data to the policy registry.
          KMOO.PolicyRegistry.registry_.byDomain[domain] = [];
        }
        KMOO.PolicyRegistry.registry_.byDomain[domain].push(el);
      }
